...
1. Reconstruct the entire `.tar.gz` from the split files.
```shell
cat miogatto.tar.gz.part-* > miogatto_reconstructed.tar.gz
```
2. Check the `sha256sum`. The sum should be as follows.
```
dac1b94f7140c1f76a80320a6eab67f775bd5c854bfdaaf9c5626e4d86e1e3e3  miogatto_reconstructed.tar.gz
```
3. Load the `.tar.gz` to Docker
```shell
docker load < miogatto_reconstructed.tar.gz
```
4. Run the Docker Image.
```sh
docker run -p 4100:4100 -d ghcr.io/lrec-coling-2024-anon42/miogatto:latest
```
...
```sh
docker run -p 4100:4100 -d ghcr.io/lrec-coling-2024-anon42/miogatto:latest python -m server <PAPER_ID>
```
...
```sh
docker run -p 4100:4100 -d ghcr.io/lrec-coling-2024-anon42/miogatto:latest python -m server 1906.05149-StableBeluga2
```
